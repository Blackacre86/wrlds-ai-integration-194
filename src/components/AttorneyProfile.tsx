...
const AttorneyProfile = () => {
  try {
    console.log('[AttorneyProfile] Rendering AttorneyProfile component');
    const { getContent } = useContentContext();
    const attorneyContent = getContent('attorney');
    
    console.log('[AttorneyProfile] Attorney content loaded:', attorneyContent);

    // Safe fallbacks
    const name = attorneyContent?.name || "Joe Brava";
    const description = attorneyContent?.description || "With 8+ years of legal experience and a proven track record of success, Attorney Joe Brava brings strategic thinking and tenacious advocacy to every case.";
    ...
    console.error('[AttorneyProfile] Error rendering AttorneyProfile component:', error);
    return (
      <section id="attorney-profile" className="py-12 md:py-16 px-4 md:px-12 bg-white">
        <div className="container mx-auto max-w-6xl">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="text-gray-800 text-lg leading-relaxed">
              Experienced criminal defense attorney serving Massachusetts.
            </p>
          </div>
        </div>
      </section>
