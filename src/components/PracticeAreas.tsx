...
const PracticeAreas = () => {
  ...
  return <section id="practice-areas" className="py-12 md:py-24 px-4 md:px-12 bg-white">
      <article itemScope itemType="https://schema.org/LegalService">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {practiceContent?.areas?.map((area: any, index: number) => <Card key={index} className="bg-gray-50 border-gray-200 hover:shadow-xl hover:-translate-y-2 transition-all duration-500 group overflow-hidden">
              <CardContent className="p-6 flex flex-col h-full">
                <div className="mb-4 overflow-hidden rounded-lg">
                  <img src={area.image} alt={area.title} className="w-full h-48 object-cover rounded-lg grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-500" />
                </div>
