...
export const ContentProvider: React.FC<ContentProviderProps> = ({ children }) => {
  const getContent = (path: string) => {
    try {
      console.log(`[ContentProvider] Fetching content for path: ${path}`);
      ...
      if (!content) {
        console.warn(`[ContentProvider] Content not found for path: ${path}`);
        return null;
      }

      console.log(`[ContentProvider] Successfully retrieved content for: ${path}`);
      return content;
    } catch (error) {
      console.error(`[ContentProvider] Error fetching content for path ${path}:`, error);
      return null;
    }
  };
...
export const useContentContext = () => {
  ...
  if (context === undefined) {
    console.error('[ContentProvider] useContentContext must be used within a ContentProvider');
